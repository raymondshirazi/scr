...
##options
# nsf does not search sub folders
...
#V0.9.11.3   Bugfixes
#V0.9.11.2   Support NumGrad & info on ongoing Freq calculations, better output detection
#V0.9.11.1   Bugfix update for orca 4.1
#V0.9.11.0beta
...
ifspe () {
  ...
  /Energy.*Gradient Calculation/ { status="true";exit 0 }
...
ifopt () {
  awk '
  /Geometry Optimization Run/ { status="true";exit 0 } #optimization run
  /Energy.*Gradient Calculation/ { exit 1}
...
read method basis mult app nsr rops <<< $(awk '
    BEGIN { if(FILENAME~/.inp/) input=1;}
    /WARNINGS/{warn=1}
    warn &&
    /RHF/{rorb8="RHF"}
    warn &&
    ...
    /!/ {for(i=1;i<=NF;i++) {
      ...
      if(tolower($i)~/tightpno/) iDLPNO="T";
      if(tolower($i)~/loosepno/) iDLPNO="L";
      if(tolower($i)~/normalpno/) iDLPNO="N";
      }
      }
    input &&
    /!/ {for(i=1;i<=NF;i++) {
      if(tolower($i)~/pc-/) {if ($i!~"/"  ) basis=$i;}
      if(tolower($i)~/sto-/) {if ($i!~"/"  ) basis=$i;}
      ...
      if(tolower($i)~/lanl/) {if ($i!~"/"  ) basis=$i;}
  ...
  input &&
  / [Nn][Uu][Mm][Ff][Rr][Ee][Qq]/{rorb9="NumFreq"}
  input &&
  / [Nn][Uu][Mm][Gg][Rr][Aa][Dd]/{rorb10="NumGrad"}
  input &&
  ...
  /END OF INPUT/ { iEND=1;input="0"; if (!rorb8 && !mrci && !mrcc && !mc ){rorb8="RHF"} else if(!rorb8 && iMULT!=1 && !mrcc && !mc){rorb8="UHF"}
                   if (method~/!/){method=substr(method,2)};if(iDLPNO){method=iDLPNO"-"method}
                   ...
                   rorb=rorb1 rorb3 rorb4 rorb5 rorb6 rorb7 rorb9 rorb10;
                   if(rorb8 && rorb){rorb= rorb "-"rorb8}else if(!rorb && rorb8){rorb=rorb8}
                   ...
                   if(mrci){method="MRCI-"citype"-"block[1,2]"-"block[1,4]}
...
if containsElement "ex" "${args[@]}" ; then #if ex is asked publish CAS energy under S^2 value
  ...
  FOUND &&
  /FINAL ENERGY EVALUATION AT THE STATIONARY POINT/{getline; if ($3 <= 4) printf "%s%s%s\t", "Warning ",$3," cycles"}
  ...
  /^T1/{T1=$NF}
    ...
    FOUND && ecomp && CC &&
  /Total.*Conventional/ {CC=2;trip0=$NF;print hi}
  ...
  /^FINAL.*ERGY/  {
      if (!PROB) # if you have energy but with SCF isuue you get ! after your energy
          {   if ( mc ){printf "%s\t","Mult-Ref"}
              ...
              else if( ecomp && CC==2 && local) {printf "%s\t%s%s%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s",S2,spair,"/",tpair,$NF,HF,MPpair,CCpair,Corr,T1,Amp,trip0,trip}
...
if containsElement "pair" "${args[@]}" ; then
  ...
  /^INITIAL GUESS ORBITALS/,/^TOTAL SCF ENERGY/ || /CASSCF RESULTS/{next}
  /^ORBITAL ENERGIES/,/MULLIKEN POPULATION ANALYSIS/ {next}
  #/MULLIKEN POPULATION ANALYSIS/,/TIMINGS/||/EXCITATION SPECTRA/||/ECORR is/{next}
  ...
  /An error has occured in the MDCI module/ {MDCI=1} #MDCI issue
  /The Coupled-Cluster iterations have NOT converged/ {printf "coupled cluster not converged"; exit}
  ...
  LED &&
  /FINAL SUMMARY DLPNO-CCSD ENERGY DECOMPOSITION/ {LEDs=1}

  LED && LEDs &&
  /Intra fragment/{ printf "%s\t", $5}

  LED && LEDs &&
  /Electrostatics \(REF.\)/{ printf "%s\t", $3}

  LED && LEDs &&
  /Exchange \(REF.\)/{ printf "%s\t", $3}

  LED && LEDs &&
  /Dispersion \(strong pairs\)/{ printf "%s\t", $4}

  LED && LEDs &&
  /Dispersion \(weak pairs\)/{ printf "%s\t", $4} 

  LED && LEDs &&
  /Non dispersion \(strong pairs\)/{ printf "%s\t", $5} 

  LED && LEDs &&
  /Non dispersion \(weak pairs\)/{ printf "%s\t", $5} 

  LED && LEDs &&
  /#Time for LED analysis/ {LEDs=""}
  ...
  ecomp && CC &&
  /The Conventional/ {CC=2;trip0=$NF}
  ...
  /^FINAL SI.*ERGY/  {
      if (!PROB) # if you have energy but with SCF isuue you get ! after your energy
          {   if ( mc ){if(CAS){printf "%s\t",CAS}else{printf "%s\t","Mult-Ref"}}
              ...
              if (CC=="0") {printf "%s\t%s","CC not converged",$5}
              else if( SCF=="0" ) {printf "%s\t%s","SCF not converged",$5}
              else if( mkccp ){if (ecomp){printf("%s ", rootn-2);for (x = 1; x <= 10; x++) {printf("%s ", Heff[x])};printf "%s\t%s\t%s\t%s",S2,$5,mkCCSD,mktriples

                               }else{printf("%s ", rootn-2);for (x = 1; x <= 10; x++) {printf("%s ", Heff[x])};printf "%s\t%s",S2,$5}}
            ...
            }else if(new_S2){printf "%s\t%s\t%s",S2,new_S2,$5
            }else if(icorr){ if(ppairs){printf "%s\t%s\t%s\t%s",S2,spair/tpair,icorr,$5}else{printf "%s\t%s\t%s",S2,icorr,$5}
            }else{printf "%s\t%s",S2,$5}}
            else if( ecomp && CC==1 && !local) {printf "%s\t%s\t%s\t%s\t%s\t%s",S2,$5,HF,Corr,T1,Amp}
            else if( ecomp && CC==1 && local) {printf "%s\t%s%s%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s",S2,spair,"/",tpair,$5,HF,MPpair,CCpair,Corr,T1,Amp}
            else if( ecomp && CC==2 && !local) {printf "%s\t%s\t%s\t%s\t%s\t%s\t%s",S2,$5,HF,Corr,T1,Amp,trip}
            else if( ecomp && CC==2 && local) {printf "%s\t%s%s%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s",S2,spair,"/",tpair,$5,HF,MPpair,CCpair,Corr,T1,Amp,trip0,trip}
              else if( ecomp && MP==1) {printf "%s\t%s\t%s\t%s",S2,$5,HF,MCorr}
              else if( ecomp && OO==1) {printf "%s\t%s\t%s\t%s",S2,$5,oHF,oMCorr}
                ...
                printf "%s\t%s\t%s",S2,$5,"none"}}
              CONV=1
          }
          }
  /ORCA NUMERICAL FREQUENCIES/ {wff=1;} #mark freq start
  /ORCA NUMERICAL GRADIENT/ {wfg=1} #mark grad start
 ...
 /.*ORCA TERMINATED NORMALLY.*/{otn=1}


  END         { if(otn!=1 && wff==1 || otn!=1 && wfg==1) {printf "\twaiting for freq or grad "}
...
if [[ -z ${args[0]} ]]; then
     ...
     echo "For help about current functions enter argument h: bash scr.sh h"
fi
...
if containsElement "All" "${args[@]}" ; then
  ...
  if containsElement "prune" "${args[@]}" ; then # no subfolders
    ...
    if containsElement "only" "${args[@]}" ; then
      find "$dir" -maxdepth 1 -name ${1}*.out | while read FILENAME
        do
          process "$FILENAME"
        done
    elif containsElement "only2" "${args[@]}" ; then
      find "$dir" -maxdepth 1 -name ${1}*${2}*.out | while read FILENAME
        do
          process "$FILENAME"
        done
    elif containsElement "only3" "${args[@]}" ; then
      find "$dir" -maxdepth 1 -name *${1}*.out | while read FILENAME
        do
          process "$FILENAME"
        done
    else
      find "$dir" -maxdepth 1 -name '*.out' | while read FILENAME
        do
          process "$FILENAME"
        done
    fi
